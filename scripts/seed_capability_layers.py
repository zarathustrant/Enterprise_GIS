...
def fixtures() -> list[LayerFixture]:
    facilities = [
        ('Central Hospital', 'health', 92, 'operational', 4.540, 8.482),
        ('Unity School', 'education', 73, 'operational', 4.558, 8.490),
        ('Fire Station', 'emergency', 88, 'maintenance', 4.576, 8.474),
        ('Water Plant', 'utility', 61, 'operational', 4.592, 8.501),
        ('Community Market', 'commercial', 55, 'planned', 4.614, 8.486),
        ('Civic Centre', 'government', 79, 'operational', 4.626, 8.516),
    ]
    facility_features = [
        ({'type': 'Point', 'coordinates': [x, y]}, {
            'name': name, 'category': category, 'score': score, 'status': status,
            'priority': index + 1, 'opened': f'202{index}-01-15',
        })
        for index, (name, category, score, status, x, y) in enumerate(facilities)
    ]
    stats_features = []
    for index, (x, y, value) in enumerate([
        (4.532, 8.470, 12), (4.536, 8.474, 15), (4.540, 8.478, 18),
        (4.590, 8.510, 72), (4.594, 8.514, 80), (4.598, 8.518, 88),
        (4.640, 8.466, 30), (4.650, 8.500, 42), (4.620, 8.530, 55),
    ]):
    ...
    return [
        LayerFixture(
            ...
            [('name', 'Name', 'string'), ('category', 'Category', 'string'), ('score', 'Score', 'double'), ('status', 'Status', 'string'), ('priority', 'Priority', 'integer'), ('opened', 'Opened', 'date')],
            facility_features,
            {
                ...
                'iconSize': 28, 'labelField': 'name', 'labelSize': 12, 'labelHaloColor': '#ffffff',
        ...
        LayerFixture(
            ...
            road_features,
            {
                ...
                'labelRotateWithLine': True, 'labelRepeatDistanceMeters': 1800,
        ...
        LayerFixture(
            ...
            district_features,
            {
                ...
                'labelField': 'name', 'labelPolygonFitEnabled': True, 'labelWrapLength': 12,
...
def seed_layer(cur, owner_id: str, fixture: LayerFixture, z_index: int) -> str:
    cur.execute(
        """
        INSERT INTO layers (
            name, description, geometry_type, crs, style, is_public, group_name, z_index, created_by
        ) VALUES (%s, %s, %s, 'EPSG:4326', %s::jsonb, TRUE, %s, %s, %s::uuid)
        RETURNING id
        """,
        (fixture.name, fixture.description, fixture.geometry_type, json.dumps(fixture.style), DEMO_GROUP, z_index, owner_id),
    )
    ...
    try:
        with connection:
            with connection.cursor() as cur:
                ...
                layer_ids = {
                    fixture.name: seed_layer(cur, owner_id, fixture, 100 - index)
