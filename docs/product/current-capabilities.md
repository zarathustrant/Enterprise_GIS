...
The vector analysis subsystem has a server-owned tool registry, durable run history, shared execution environments, asynchronous job linkage, output schema propagation, warnings, and timing metrics. Buffer supports all-feature or selected-feature scope plus optional precision-grid snapping, while Multi-Ring Buffer creates ordered geodesic rings or cumulative disks. Intersect preserves prefixed schemas and domains from both inputs, records source feature IDs, infers or explicitly extracts output geometry dimensions, supports independent input selections, and uses an indexed single-calculation overlay kernel. Clip and Erase use polygon masks while preserving source schema, style, geometry family, feature provenance, selection environments, and transactional safety. Dissolve supports multi-field grouping, null policies, multipart or singlepart output, and seven schema-aware statistics. Spatial Join supports eight indexed predicates, two cardinality modes, deterministic matching, keep-all behavior, prefixed schemas, and join counts. Summarize Within provides explicit boundary semantics, geodesic length/area measurements, grouped categories, numeric statistics, percentages, and empty-zone retention. Near generates 1-100 ranked relationships with indexed candidates, spheroid distance and bearing, closest-point coordinates, optional distance limits, self-exclusion, and three visual output geometries. Polygonize builds polygons from noded line networks, transfers attributes, and emits unconsumed-edge diagnostics. The original Within query remains on the legacy path.
